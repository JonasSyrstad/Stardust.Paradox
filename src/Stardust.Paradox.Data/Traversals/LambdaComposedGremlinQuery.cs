...
namespace Stardust.Paradox.Data.Traversals
{
    public class LambdaComposedGremlinQuery : ComposedGremlinQuery
    {
        ...
        private readonly Func<GremlinQuery, GremlinQuery>[] _inner2;

        internal LambdaComposedGremlinQuery(GremlinQuery queryBase, string query,
            params Func<GremlinQuery, string>[] inner) : base(queryBase, query)
        {
            _inner = inner;
        }

        internal LambdaComposedGremlinQuery(GremlinQuery queryBase, string query,
            params Func<GremlinQuery, GremlinQuery>[] inner) : base(queryBase, query)
        {
            _inner2 = inner;
        }
