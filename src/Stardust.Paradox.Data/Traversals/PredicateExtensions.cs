...
namespace Stardust.Paradox.Data.Traversals
{
    public static class PredicateExtensions
    {
        ...
        {
            return new LambdaComposedGremlinQuery(queryBase, "is({0})",
        ...
        /// <param name="expression">the filter expression</param>
        /// <returns></returns>
        public static GremlinQuery Not(this GremlinQuery queryBase,
            Func<PredicateGremlinQuery, GremlinQuery> expression)
        {
            return new LambdaComposedGremlinQuery(queryBase, "not({0})",
                query => expression.Invoke(new PredicateGremlinQuery(query)).CompileQuery());
        }
        ...
        public static GremlinQuery And(this GremlinQuery queryBase,
            Func<PredicateGremlinQuery, GremlinQuery> expression)
        {
            return new LambdaComposedGremlinQuery(queryBase, "and({0})",
                query => expression.Invoke(new PredicateGremlinQuery(query)).CompileQuery());
        }
        ...
        /// <returns></returns>
        public static GremlinQuery And(this GremlinQuery queryBase,
            Func<PredicateGremlinQuery, GremlinQuery> expression1,
            Func<PredicateGremlinQuery, GremlinQuery> expression2)
        {
            return new LambdaComposedGremlinQuery(queryBase, "and({0},{1})",
        ...
        /// <summary>
        ///     The or()-step ensures that at least one of the provided traversals yield a result (filter). Please see and() for
        ///     and-semantics.
        /// </summary>
        /// <param name="queryBase">the base query to append the step to</param>
        /// <param name="expression">the filter expression</param>
        ...
        /// <summary>
        ///     The optional()-step (branch/flatMap) returns the result of the specified traversal if it yields a result else it
        ///     returns the calling element, i.e. the identity().
        /// </summary>
        /// <param name="queryBase">the base query to append the step to</param>
        /// <param name="expression">the filter expression</param>
        /// <returns></returns>
        public static GremlinQuery Optional(this GremlinQuery queryBase,
            Func<PredicateGremlinQuery, GremlinQuery> expression)
        {
            return new LambdaComposedGremlinQuery(queryBase, "optional({0})",
                query => expression.Invoke(new PredicateGremlinQuery(query)).CompileQuery());
        }
            ...
            Func<PredicateGremlinQuery, GremlinQuery> expression)
        {
            return new ComposedGremlinQuery(queryBase,
                $"has({queryBase.ComposeParameter(key)},{expression.Invoke(new PredicateGremlinQuery(queryBase))})");
        }
        ...
        {
            return new ComposedGremlinQuery(queryBase,
                $"inside({queryBase.ComposeParameter(start)},{queryBase.ComposeParameter(end)})");
        }
        ...
        {
            return new ComposedGremlinQuery(queryBase,
                $"outside({queryBase.ComposeParameter(start)},{queryBase.ComposeParameter(end)})");
        }
