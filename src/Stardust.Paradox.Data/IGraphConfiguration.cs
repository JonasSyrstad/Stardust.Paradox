...
namespace Stardust.Paradox.Data
{
    public interface IGraphConfiguration<T> where T : IGraphEntity
    {
        ...
        InReverse<TReverse, T> In<TReverse>(Expression<Func<T, IEdgeNavigation<TReverse>>> inPropertyLambda)
            where TReverse : IVertex;

        InReverse<TReverse, T> In<TReverse>(Expression<Func<T, IEdgeNavigation<TReverse>>> inPropertyLambda,
            string label) where TReverse : IVertex;

        OutReverse<TReverse, T> Out<TReverse>(Expression<Func<T, IEdgeNavigation<TReverse>>> inPropertyLambda)
            where TReverse : IVertex;
        ...
        IGraphConfiguration<T> AddQuery(Expression<Func<T, object>> inPropertyLambda,
    ...
    public class InReverse<T, TParent> where T : IVertex where TParent : IGraphEntity
    {
        ...
        private readonly IGraphConfiguration<TParent> _c;
        ...
        public IGraphConfiguration<TParent> Out(Expression<Func<T, object>> func)
        {
            ...
            if (t.TryGetValue(prop.Member, out var def))
                throw new ArgumentOutOfRangeException(func.Name, "binding is already added");
