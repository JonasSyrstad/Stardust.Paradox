...
{
    internal class GraphConfiguration<T> : GraphConfigurationBase, IGraphConfiguration<T>, IEdgeConfiguration<T>
        where T : IGraphEntity
    {
            ...
            bool eagerLoading)
        {
            ...
            if (t.TryGetValue(prop.Member, out var def))
                throw new ArgumentOutOfRangeException(inPropertyLambda.Name, "binding is already added");
        ...
        {
            return Reverse(inPropertyLambda);
        }

        public IGraphConfiguration<T> Out<TReverse>(Expression<Func<TReverse, object>> inPropertyLambda,
            bool eagerLoading)
        {
            return Reverse(inPropertyLambda, eagerLoading);
        }
        ...
        public IOutEdgeConfiguration<T> AddOutEdge(Expression<Func<T, object>> outPropertyLambda, string label)
        {
            ...
            if (t.TryGetValue(prop.Member, out var def))
                throw new ArgumentOutOfRangeException(outPropertyLambda.Name, "binding is already added");
            t.Add(prop.Member, new FluentConfig
            {
        ...
        public InReverse<TReverse, T> In<TReverse>(Expression<Func<T, IEdgeNavigation<TReverse>>> inPropertyLambda)
        ...
        public OutReverse<TReverse, T> Out<TReverse>(Expression<Func<T, IEdgeNavigation<TReverse>>> inPropertyLambda)
            where TReverse : IVertex
        {
            var prop = inPropertyLambda.Body as MemberExpression;
        ...
        public IGraphConfiguration<T> AddQuery(Expression<Func<T, object>> inPropertyLambda,
    ...
    {
        private static readonly Dictionary<string, Type> _pendingEntityDefinitions = new Dictionary<string, Type>();
        ...
        public void BuildModel()
        {
            foreach (var pendingEntityDefinition in _pendingEntityDefinitions)
                if (!GraphContextBase._dataSetLabelMapping.ContainsKey(pendingEntityDefinition.Value))
                {
                    ...
                    GraphContextBase._dataSetLabelMapping.Add(pendingEntityDefinition.Value,
                        pendingEntityDefinition.Key);
                }
