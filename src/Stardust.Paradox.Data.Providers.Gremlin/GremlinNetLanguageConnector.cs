...
namespace Stardust.Paradox.Data.Providers.Gremlin
{
    public class GremlinNetLanguageConnector : LanguageConnectorBase, IGremlinLanguageConnector //, IDisposable
    {
        ...
        private static readonly ConcurrentDictionary<string, GremlinClient> gremlinClients =
            new ConcurrentDictionary<string, GremlinClient>();
        ...
        public GremlinNetLanguageConnector(string gremlinHostname, string databaseName, string graphName,
            string accessKey, ILogging logger = null) : base(logger)
        {
            CanParameterizeQueries = true;
            _key = $"{gremlinHostname.ToLower()}.{databaseName.ToLower()}.{graphName.ToLower()}";
            _server = new GremlinServer(gremlinHostname, 443, true, $"/dbs/{databaseName}/colls/{graphName}",
                accessKey);
            InitializeClient();
        }
            ...
            while (executeQuery)
                ...
                catch (AggregateException aggregateException)
                {
                    if (aggregateException.InnerException is IOException ||
                        aggregateException.InnerException is ServerUnavailableException ||
                        aggregateException.InnerException is NoConnectionAvailableException)
                    {
        ...
        public bool CanParameterizeQueries { get; set; }
        ...
        private async Task HandleConnectionException(Exception ioException)
        {
            ...
            lock (lockObject)
            {
                if (gremlinClients.TryRemove(_key, out var c)) c?.Dispose();
            }
