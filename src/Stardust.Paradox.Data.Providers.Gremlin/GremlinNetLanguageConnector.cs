...
namespace Stardust.Paradox.Data.Providers.Gremlin
{
    public class GremlinNetLanguageConnector : LanguageConnectorBase, IGremlinLanguageConnector//, IDisposable
    {
        ...
        private void InitializeClient()
        {
            ...
            {
                if (gremlinClients.TryGetValue(_key, out _client)) return;
                _client = new GremlinClient(_server, new GraphSON2Reader(),
                    new GraphSON2Writer(), GremlinClient.GraphSON2MimeType, ConnectionPoolSettings = ConnectionPoolSettings, WebSocketConfiguration= WebSocketConfiguration);
        ...
        public virtual async Task<IEnumerable<dynamic>> ExecuteAsync(string compileQuery, Dictionary<string, object> parametrizedValues)
        {
            ...
            while (executeQuery && retry <= maxRetries)
            {
                try
                {
                ...
                catch (AggregateException aggregateException)
                {
                    if (aggregateException.InnerException is IOException || aggregateException.InnerException is ServerUnavailableException || aggregateException.InnerException is NoConnectionAvailableException)
                    {
                        ...
                        if (retry > maxRetries)
                        {
                            Log($"Maximum retry attempts ({maxRetries}) exceeded for connection exception: {aggregateException.InnerException.Message}");
                            throw;
                        }
                    }
                    if (aggregateException.InnerException is ResponseException rex)
                    {
                        ...
                        if (retry > maxRetries)
                        {
                            Log($"Maximum retry attempts ({maxRetries}) exceeded for response exception: {rex.Message}");
                            throw;
                        }
                    }
                }
                catch (IOException ioException)
                {
                    ...
                    if (retry > maxRetries)
                    {
                        Log($"Maximum retry attempts ({maxRetries}) exceeded for IO exception: {ioException.Message}");
                        throw;
                    }
                }
                catch (NoConnectionAvailableException ioException)
                {
                    ...
                    if (retry > maxRetries)
                    {
                        Log($"Maximum retry attempts ({maxRetries}) exceeded for no connection available exception: {ioException.Message}");
                        throw;
                    }
                }
                catch (ResponseException responseException)
                {
                    ...
                    if (retry > maxRetries)
                    {
                        Log($"Maximum retry attempts ({maxRetries}) exceeded for response exception: {responseException.Message}");
                        throw;
                    }
                }
            ...
            throw new Exception($"Maximum retry attempts ({maxRetries}) exceeded");
        }
