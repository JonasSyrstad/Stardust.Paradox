...
namespace Stardust.Paradox.Data.InMemory.Tests.CosmosDbMigrated
{
    ...
    public class GremlinTests : IDisposable
    {
        ...
        public async Task InsertItem()
        {
            ...
            {
                var jonas = CreateItem(tc, "Jonas", "Technical Solution Architect", true);
                jonas.ProgramingLanguages.Add("C#");
                jonas.ProgramingLanguages.Add("C++");
                jonas.ProgramingLanguages.Add("JavaScript");
                jonas.ProgramingLanguages.Add("Gremlin");
            ...
            using (var tc= TestContext())
            {
                var sanne = await tc.Profiles.GetAsync("Sanne");
                var parents = await sanne.Parents.ToVerticesAsync();
                Assert.NotEmpty(parents);
                Assert.Equal(2,parents.Count());
                
                // Verify that the parent-child edges exist and can be queried
                var edgeResults = await tc.ExecuteAsync<object>(g => g.V("Sanne").InE("parent"));
        ...
        public async Task GetEdges()
        {
            using (var tc = TestContext())
            {
                var s = await tc.Profiles.GetAsync(CreateTuple("Sanne"));
                if (s != null)
                {
        ...
        public async Task DataContextReadTestAsync()
        {
            using (var tc = TestContext())
            {
                var jonas = await tc.VAsync<IProfile>("Jonas", "Jonas");
                if (jonas == null)
                {
                    _output.WriteLine("Jonas profile not found - skipping test");
                    return;
                }
                ...
                if (children.Any())
                {
                    ...
                    var employers = await jonas.Employers.ToVerticesAsync();
                    if (employers.Any())
                    {
                        var gssit = employers.First();
        ...
        public async Task GetTreeTest()
        {
            using (var tc = TestContext())
            {
                try
                {
                    ...
                    var jonas = await tc.GetTreeAsync<IProfile>("Jonas", t => t.Parents, true);
                    _output.WriteLine(JsonConvert.SerializeObject(jonas));
                }
                catch (Exception ex)
                {
        ...
        public async Task DataContextReadWriteTestAsync()
        {
            ...
            using (var tc = TestContext())
            {
                var jonas = await tc.VAsync<IProfile>(CreateTuple("Jonas"));
                if (jonas == null)
                {
                    jonas = tc.CreateEntity<IProfile>("Jonas");
                    jonas.Name = "Jonas";
                    jonas.Pk = "Jonas";
                    await tc.SaveChangesAsync();
                }
        ...
        public async Task GraphSetTests()
        {
            using (var tc = TestContext())
            {
                try
                {
                    ...
                    if (jonas == null)
                    {
                        _output.WriteLine("Jonas not found - creating for test");
                        jonas = tc.CreateEntity<IProfile>("Jonas");
                        jonas.Name = "Jonas";
                        jonas.Pk = "Jonas";
                        await tc.SaveChangesAsync();
