    ...
    public class GremlinTests : IDisposable
    {
        ...
        private TestContext TestContext()
        {
            JsonConvert.DefaultSettings = () =>
            {
                return new JsonSerializerSettings
                {
                    ...
                    Error = (sender, args) => { Logging.DebugMessage(""); }
                }.RegisterGraphSerializer();
            };
            //var tc = new TestContext(new Class1());
            var tc = new TestContext(new GremlinNetLanguageConnector(
                $"{ConfigurationManagerHelper.GetValueOnKey("cosmosDbAccount")}.gremlin.cosmosdb.azure.com",
                "graphTest", "services", ConfigurationManagerHelper.GetValueOnKey("cosmosDbKey")));
            tc.OnDisposing = c =>
            {
        ...
        public async Task DataContextReadTestAsync()
        {
            ...
            using (var tc = TestContext())
            {
                ...
                _output.WriteLine(
                    JsonConvert.SerializeObject(await (await gss.Group.ToVertexAsync()).AllEmployees
                        .ToVerticesAsync()));
            }
        }
        ...
        public async Task ExecuteAdvancedQueryTest()
        {
            ...
            using (var tc = TestContext())
            {
                ...
                var eq = await tc.Profiles.GetTypedAsync(g =>
                    g.V().Has(t => t.Name, "Sanne").As("s").In(t => t.Parents).Out(t => t.Children)
                        .Where(p => p.Without("s")).Dedup());
        ...
        public async Task GraphSetTests()
        {
            ...
            using (var tc = TestContext())
            {
                ...
                foreach (var profile in nochildren) _output.WriteLine("wtf?!?");
                _output.WriteLine(
                    JsonConvert.SerializeObject(await jonas2.SingleOrDefault().GetTreeAsync<IProfile>(p => p.Parents)));
            }
        }
        ...
        public async Task InsertItem()
        {
            ...
            using (var tc = TestContext())
            {
                ...
                sanne.Parents.Add(jonas,
                    new Dictionary<string, object> {{"birthPlace", "Kristiansand"}, {"created", DateTime.Now}});
        ...
        public async Task QueryBuilderTest()
        {
            ...
            var z = v.HasId("Jonas").Has("name", "Jonas")
                .Has("lastUpdatedEpoch", p => p.Gte(val)); //.Has("lastUpdated", p => p.Gte((decimal)50.3));
            ...
            Assert.Equal("g.V().has(__p0,__p1).as(__p2).in(__p3).out(__p4).where(without(__p6)).dedup().values(__p5)",
                siblings.ToString());
