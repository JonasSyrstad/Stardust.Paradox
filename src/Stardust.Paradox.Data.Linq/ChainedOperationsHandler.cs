...
Type elementType = null;
 ...
 elementType = genericArgs[0];
  ...
  // DEBUG: Count items in projected data
    int itemCount = 0;
    foreach (var item in enumerable)
    {
  itemCount++;
    }
  Console.WriteLine($"[ChainedOperationsHandler] Projected data count: {itemCount}, Type: {projectedData.GetType().Name}");
...
extractor.Visit(expression);
    ...
    Console.WriteLine($"[ChainedOperationsHandler] Operations count: {extractor.Operations.Count}");
    foreach (var op in extractor.Operations)
 {
        Console.WriteLine($"[ChainedOperationsHandler]   - {op.GetType().Name}");
 }
...
return projectedData;
  ...
  var queryable = asQueryableMethod.MakeGenericMethod(elementType).Invoke(null, new[] { projectedData });
    ...
    {
        Console.WriteLine($"[ChainedOperationsHandler] Applying {operation.GetType().Name}");
  currentResult = operation.Apply(currentResult, currentElementType);

      // Count items after operation
     if (currentResult is IEnumerable resultEnum)
        {
      int resultCount = 0;
 ...
 Console.WriteLine($"[ChainedOperationsHandler]   Result count: {resultCount}");
        }
