...
new GremlinQueryProvider(graphSet.Context, label));
 ...
 /// <param name="source">The source queryable</param>
  ...
  Expression.Quote(predicate));
    ...
    /// </summary>
/// <typeparam name="TResult">The result type (vertex or edge)</typeparam>
 /// <param name="source">The source queryable</param>
 /// <param name="label">The label to select from</param>
      /// <returns>An IQueryable of the selected type</returns>
 public static IQueryable<TResult> Select<TResult>(this IQueryable source, string label)
   where TResult : IGraphEntity
        {
        if (source == null) throw new ArgumentNullException(nameof(source));
     if (string.IsNullOrEmpty(label)) throw new ArgumentException("Label cannot be null or empty", nameof(label));

  var method = typeof(GraphSetLinqExtensions)
 .GetMethods(BindingFlags.Public | BindingFlags.Static)
.First(m => m.Name == nameof(Select) && 
         ...
         if (propertyInfo == null)
{
   ...
   // First, try to get the edge label from the fluent configuration using CodeGenerator
        var propertyKey = $"{propertyInfo.DeclaringType.FullName}.{propertyInfo.Name}";
 var fluentLabel = CodeGeneration.CodeGenerator.GetEdgeLabel(propertyKey);
 if (fluentLabel != null)
    {
       return fluentLabel;
     }
